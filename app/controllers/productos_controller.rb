class ProductosController < ApplicationController
  ...
  def edit
    if @producto.nil?
      flash[:error] = "Product not registered"
      redirect_to productos_path  # O cualquier otra acción para redirigir, como a un listado de productos
    end
  end
  ...
  private
    ...
    def set_producto
      ...
      if @producto.nil?
        flash[:error] = "Product not registered"
        redirect_to productos_path # Redirige si no se encuentra el producto
      end
    end

    # Only allow a list of trusted parameters through.
    def producto_params
      params.require(:producto).permit(:name, :price, :description)
    end
end
